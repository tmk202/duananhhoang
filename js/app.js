...
function syncFloatContactLinks(site = {}) {
  const root = document.getElementById("floatContact");
  if (!root) return;
  root.classList.add("is-open", "float-contact--always-open");
  ...
  root.querySelectorAll("a[data-messenger]").forEach((a) => {
    a.href = messenger;
  });
...
async function boot() {
  ...
  initHeaderScroll();
  initScrollReveal();
  // Ensure float visible even if site.json fails
  syncFloatContactLinks();
  try {
    const { products, categories, site } = await loadAllData();
    window.VEX = { products, categories, site };
    initSiteLinks(site);
    syncFloatContactLinks(site);
